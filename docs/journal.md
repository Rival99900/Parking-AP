# 📓 Journal de bord — Parking-AP

> Suivi chronologique du travail de l'équipe. Une ligne par action importante.

## Équipe

| Membre | Rôle |
|---|---|
| Albert | Dépôt GitHub + plan du site |
| Ibrahima | Préparation de la BDD + MCD |
| Chris-Elliot | Maquettes |

## Tache 1 — Documentation (échéance : 28/09/2026)

| Date | Membre | Action | Statut |
|---|---|---|---|
| 21/09/2026 | Équipe | Lecture du sujet et répartition des tâches en 3 rôles | ✅ Fait |
| 21/09/2026 | Équipe | Création du projet Asana « Projet Parking » (À faire / En cours / Terminé) | ✅ Fait |
| 21/09/2026 | Albert | Création du dépôt GitHub `Parking-AP` (branche `main`) | ✅ Fait |
| 21/09/2026 | Albert | Ajout de `docs/taches.md` (équipe et responsabilités) | ✅ Fait |
| 21/09/2026 | Albert | Rédaction de `docs/plan-du-site.md` (arborescence + URLs) | ✅ Fait |
| … | Ibrahima | MCD (entités, associations, cardinalités, règles de gestion) | ⏳ À faire |
| … | Ibrahima | Préparation de la base de données | ⏳ À faire |
| … | Chris-Elliot | Maquettes front-office (desktop + mobile) | ⏳ À faire |
| … | Chris-Elliot | Maquettes back-office (desktop + mobile) | ⏳ À faire |
| … | Équipe | Relecture croisée MCD / maquettes / plan du site | ⏳ À faire |

## Décisions prises

- Suivi des tâches sur Asana, version documentée dans `docs/taches.md`.
- Front-office et back-office séparés (URLs `/espace/*` et `/admin/*`).

## Difficultés rencontrées

- *Pour l'instant pas de difficulté rencontré*

## Points à trancher

- Choix du chef de projet
